...
- End-to-end send can only be verified with a real device token (needs the iOS client built). The full ring chain (PSTN -> push -> CallKit -> conference audio) is a physical-device step (Simulator can't do PushKit/CallKit) — see `ios/README.md` §5.
- Sandbox-vs-production APNs host selection IS verified hermetically in `server/__tests__/iosPushChannel.test.ts` (mocks `@parse/node-apn`, asserts the `production` flag per `environment`, provider caching, VoIP payload shape, TerminalTokenError). No device needed for that bullet.
