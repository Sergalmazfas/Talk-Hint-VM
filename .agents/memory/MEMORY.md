...
- [Diagnostic recording scope](diagnostic-recording-scope.md) — only the admin-enabled diagnostic profile may record; its incoming and outgoing calls are silent, with no global switch.
...
- [Benchmark run ops](benchmark-run-ops.md) — benchmark storage = dev DB; run via dev-server HTTP + temp session row, never detached tsx scripts (they die silently).
...
- [Strategy memory (v2.2)](strategy-memory.md) — bounded per-call hint-cycle tracker; only DELIVERED hints open cycles; outcomes reuse #226 usageScore; CHOICE branch needs strict best-option win.
...
- [Copilot private translator](copilot-private-translator-boundary.md) — V1 private RU→EN text without Brain; V2 reasoning separate; private intent is not spoken or auto-cancelled by Guest.
