...
- **Idempotent retry invariant:** every PREPARE send (message AND goal confirm) carries a client-generated idempotency id; a resend after a lost ack must replay the original result (including re-delivering `goal_set` to the resending socket) — never a duplicate turn, duplicate opening, or re-fired goal side effects. Failures must NOT be cached (retry genuinely re-runs); replay state must be TTL-bounded per user. **Why:** reconnect resend ("server got it, client missed the ack") is the normal mobile failure mode, and replay caches must not leak memory per user. **How to apply:** any new PREPARE-like request/response pair needs the same id + dedup + bounded-replay treatment; keep clients to ONE in-flight turn so ack correlation stays unambiguous, and keep recorded audio/text client-side until success so nothing is lost.
