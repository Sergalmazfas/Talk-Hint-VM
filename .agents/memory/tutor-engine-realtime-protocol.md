...
- Audio-turn `TURN_FAILED` is an ENGINE-side STT failure (our pcm16@24k stream is per contract); the fix belongs in the engine project, and TalkHint must always surface it visibly (toast) — a silent failed turn reads as "tutor ignores me".
