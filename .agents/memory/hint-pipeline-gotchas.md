...
**How to apply:** compute a `suppressFiller` flag before `fastLayer.onGstUtteranceEnd()` covering every condition that returns early before the suggestion broadcast — currently goal-achieved, reaction-only, farewell, `waitingForInfo && waitAckShown` (wait-state after its single ACK), and the hint cooldown. Any NEW early-return guard added to the suggestion path must also be added here, or the filler leaks again.
