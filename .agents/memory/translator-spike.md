...
The native Translator has two isolated transports: the standalone authenticated 24 kHz PCM channel used for direct testing, and a dedicated Twilio phone bridge. PSTN routing is intentionally asymmetric: Owner hears Guest's English original followed by its Russian translation, and the exact same generated Owner→English audio instance is sent to both Guest and Owner. Guest never hears Owner's Russian source or Guest's own translation. Owner-bound playback is output-only and must not re-enter a provider session.
