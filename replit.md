...
The on-call DB save-health endpoints are `GET /api/health` (JSON data source) and `GET /health` (a self-contained HTML page that fetches `/api/health` client-side and renders it). They expose operational internals (table names, save-failure counts, schema-drift errors) but no secrets or user data. Access is gated by an **opt-in shared token** (`healthAccessAllowed` in `server/routes.ts`): the gate only restricts access **in production** AND **only when `HEALTH_STATUS_TOKEN` is set** — outside production, or with no token configured, both endpoints stay open (frictionless dev, no surprise lockout of uptime monitors). When locked, `/api/health` returns `401 {error:"Unauthorized"}` (no internals) and `/health` returns a token-prompt page (`HEALTH_TOKEN_PROMPT_HTML`) instead of the dashboard. The token may be supplied as `?token=`, an `x-health-token` header, or a `Bearer` header, and is compared in constant time. The `/health` page forwards its `?token=` to its `/api/health` fetch, so opening `/health?token=…` unlocks the live data. The `/app` sidebar "System status" link is a plain navigation (no token), so in a locked production it lands on the token prompt — operators bookmark `/health?token=…`.
