...
### Hint model & timing
The live hint generator (`translateAndSuggest`) model is configurable via the `HINT_MODEL` env var (default `gpt-4o-mini`) — switch models without a code change. The guest-utterance pipeline emits `[TIMING]` logs per turn: `gpt=<ms>` (OpenAI call duration), `reaction end_of_turn->caption=<ms>` and `reaction end_of_turn->suggestion=<ms>` (measured from `handleGuestUtteranceComplete` entry ≈ Deepgram EndOfTurn). Duplicate-suggestion blocking uses `DUPLICATE_SIMILARITY = 0.8` (Jaccard).
