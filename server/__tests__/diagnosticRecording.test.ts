...
import {
  ...
  DIAGNOSTIC_RECORDING_POLICY_VERSION,
...
describe("recording consent policy", () => {
  it("stamps the single silent policy for admin-enabled diagnostic recordings", () => {
    expect(DIAGNOSTIC_RECORDING_POLICY_VERSION).toBe("silent-diagnostic-v1");
  });
});
